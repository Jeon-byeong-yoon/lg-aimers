...
def main():
    ...
    feature_models = joblib.load("./model/v161_feature_models.joblib")
    ...
    network = joblib.load("./model/v187_network.joblib")
    extra_hierarchical = joblib.load("./model/v187_hierarchical_lookups.joblib")
    factorization = joblib.load("./model/v161_factorization.joblib")
    ...
    # V187: `hierarchical_target_encoding_v6.py` defines four groups and this pipeline
    # has passed only `pitcher_batter` since V6, where the rest were dropped on a 3e-5
    # Brier difference from a single HGB. The extra lookups are built from the training
    # seasons and merged here; only the embedding network was retrained on them, and
    # every other model selects its own stored columns by name, so the added columns
    # reach the network and are invisible to the rest.
