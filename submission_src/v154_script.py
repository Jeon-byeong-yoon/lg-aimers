"""V154: a season-tracking prior for CatBoost's features, plus an experience segment.

The in-season reconstruction shrinks toward the mean of the career as-of rate column, and
a career average lags a league that fell from 0.5647 to 0.4861. The gap grows every
season -- +0.0184, +0.0444, +0.0541 for the three folds -- and shows up as an
over-prediction that falls monotonically with pitcher experience, which is the shape a bad
prior makes because it carries weight 20/(inside_n + 20).

Only CatBoost reads the corrected features. The prior helps it on every fold and hurts
Form, so Form and the two networks keep the career prior they were fitted on; feeding a
model features built with a different prior than it trained on would be a mismatch. Hence
three reconstructions below rather than two.
"""
