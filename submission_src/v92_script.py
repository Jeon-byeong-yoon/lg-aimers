"""V92: V41 plus the current-season as-of drift correction (weight 0.20)."""
...
from inseason_asof_features_v92 import add_inseason_features, drift_correction
...
def main():
    ...
    inseason = joblib.load("./model/v92_inseason_anchor.joblib")
    ...
    # V92: the official asof_* counters are career-cumulative and continue across
    # seasons, so subtracting the frozen end-of-2024 state recovers each pitcher's
    # 2025-only control rate. Only this row's own official features and the frozen
    # training-time lookup are used, so evaluation rows never interact.
    inseason_frame = add_inseason_features(
        raw,
        inseason["anchors"],
        inseason["priors"],
    ...
    prediction = np.clip(
        prediction + drift_correction(inseason_frame, inseason["weight"]), 0, 1
    )
