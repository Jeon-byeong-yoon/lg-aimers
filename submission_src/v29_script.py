"""V29: enhanced contextual Trackman blend plus stable game-state correction."""
...
def main():
    ...
    v25_models = joblib.load("./model/v25_feature_models.joblib")
    context_model = joblib.load("./model/v29_context_model.joblib")
    calibration = joblib.load("./model/v29_calibration.joblib")
    ...
    feature_sets = {
        "extra_trees": base, "trackman_hgb": trackman_base,
        "encoded_hgb": encoded_trackman, "hierarchical_hgb": hierarchical_trackman,
    }
    tree_prediction = sum(
        TREE_WEIGHTS[name]
    ...
    form_prediction = v25_models["form_model"].predict_proba(
        form_features[v25_models["form_columns"]]
    )[:, 1]
    context_keys = add_context_keys(hierarchical_trackman)
    context_features = attach_lookup(
        context_keys, context_model["context_lookup_2025"], KEYS + CONTEXT
    )
    context_prediction = context_model["context_model"].predict_proba(
        context_features[context_model["context_columns"]]
    )[:, 1]
    prediction = 0.69 * v17_prediction + 0.12 * form_prediction + 0.19 * context_prediction
