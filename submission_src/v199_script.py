...
def main():
    ...
    # V199: four CatBoost draws. Its standalone seed range is narrow -- 1,200 boosting
    # iterations average away most of the permutation randomness behind ordered target
    # statistics -- but at the blend level it is the second widest of the five components,
    # because a small wobble multiplied by the second-largest weight is still a wobble.
    catboost_meta = joblib.load("./model/v199_catboost_meta.joblib")
    catboosts = []
    for name in catboost_meta["catboost_files"]:
        member = CatBoostClassifier()
        member.load_model(f"./model/{name}")
        catboosts.append(member)
    ...
    catboost_matrix = catboost_frame(catboost_source, raw, catboost_meta)
    catboost_prediction = np.mean(
        [member.predict_proba(catboost_matrix)[:, 1] for member in catboosts],
        axis=0,
    )
