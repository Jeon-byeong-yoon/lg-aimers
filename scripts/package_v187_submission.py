"""Package V187: give the unused hierarchical encodings to the network.

Form, Context, CatBoost and the factorization network are V161's and V154's,
byte for byte. Three artifacts are new: the embedding network retrained on all four
hierarchical encoding groups, the lookups for the three groups this pipeline has never
shipped, and the calibration bundle, refitted because the network's out-of-fold
predictions moved.

`script.py` changes by one dict merge. Every other model selects its own stored columns
by name, so the added encodings reach the network and are invisible to the rest. The
row-independence and full-frame timing checks are re-run rather than inherited.
...
def main():
    files = {
        Path("submission_src/v187_script.py"): "script.py",
        ...
        Path("artifacts/v187_calibration.joblib"): "model/v25_calibration.joblib",
        Path("artifacts/v187_network.joblib"): "model/v187_network.joblib",
        Path("artifacts/v187_hierarchical_lookups.joblib"):
            "model/v187_hierarchical_lookups.joblib",
    ...
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v187.zip")
