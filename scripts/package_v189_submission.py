"""Package V189: average the embedding network over three seeds.

Form, Context, CatBoost and the factorization network are V161's and V154's,
byte for byte, and the feature frame is V175's 105 columns exactly -- V187 lost 2.59
with the extra encoding groups, so they are not here. Two artifacts change: the three
network bundles that replace the single draw, and the calibration, refitted because the
network's out-of-fold predictions moved.

`script.py` averages three `predict` calls. Because it changed, the row-independence and
full-frame timing checks are re-run rather than inherited -- and the timing matters more
than usual now that inference runs the network three times.
...
def main():
    files = {
        Path("submission_src/v189_script.py"): "script.py",
        ...
        Path("artifacts/v189_calibration.joblib"): "model/v25_calibration.joblib",
        Path("artifacts/v189_networks.joblib"): "model/v189_networks.joblib",
    ...
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v189.zip")
