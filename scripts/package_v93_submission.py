"""Package V93: V92 plus the in-season reconstruction inside the Form HGB."""
...
def main():
    files = {
        Path("submission_src/v93_script.py"): "script.py",
        ...
        Path("artifacts/v93_feature_models.joblib"): "model/v93_feature_models.joblib",
        Path("artifacts/v93_calibration.joblib"): "model/v25_calibration.joblib",
        Path("artifacts/v92_inseason_anchor.joblib"): "model/v92_inseason_anchor.joblib",
    }
    verify_server_loadable(
        [str(source) for source in files if str(source).endswith(".joblib")]
    )
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v93.zip")
