"""V118: LightGBM as a sixth blend component, on top of V116's CatBoost.

V116 promoted `no_te_strong_cb0.14`: CatBoost trained on the Form feature set with
`te_*`/`hte_*` removed, blended at 0.14 taken from the V17 share. Its 2024 bootstrap
lower bound was +4.1 and all three seasons improved, so it becomes the new baseline
here rather than a sibling candidate.

That distinction matters. LightGBM and CatBoost are both gradient-boosted trees over
nearly the same columns, so their errors are correlated; testing LightGBM against
plain V114 would credit it for signal CatBoost already supplies. The honest question
is whether LightGBM adds anything *beyond* CatBoost, which is what a sixth-component
test measures.

LightGBM's categorical handling differs from CatBoost's: it sorts categories by
gradient statistics and takes an optimal split, which overfits high-cardinality
columns unless `cat_smooth` and `min_data_per_group` restrain it. Both configs set
those well above the defaults for that reason, and follow this project's repeated
finding that a low learning rate with many trees transfers across seasons better
than a fast fit.
"""
...
OUTPUT = Path("artifacts/v118_lightgbm_metrics.json")
PREDICTIONS = Path("artifacts/v118_lightgbm_predictions.joblib")
FEATURE_SHRINKAGE = 20.0
DRIFT_SHRINKAGE = 3.0
DRIFT_WEIGHT = 0.10
# V116 promoted blend: v17 / form / context / network / catboost
BASE = (0.05, 0.40, 0.21, 0.20, 0.14)
CATBOOST_SOURCE = "no_te_strong"
CONFIGS = {
    "gentle": dict(n_estimators=1200, learning_rate=0.03, num_leaves=63,
                   min_child_samples=100, cat_smooth=30.0, min_data_per_group=200,
                   colsample_bytree=0.9, reg_lambda=5.0),
    "strong": dict(n_estimators=2000, learning_rate=0.015, num_leaves=31,
                   min_child_samples=300, cat_smooth=100.0, min_data_per_group=500,
                   colsample_bytree=0.8, subsample=0.8, subsample_freq=1,
                   reg_lambda=20.0),
}
FEATURE_VARIANTS = ("full", "no_te")
LIGHTGBM_WEIGHTS = (0.06, 0.12, 0.18)
P = 100000.0 / 0.25


def blend(weights, oof, logistic, form, context, network, catboost, extra, frame):
    """Six-way blend; calibration refitted exactly as every earlier step did."""
    w_v17, w_form, w_context, w_network, w_catboost, w_extra = weights
    raw = {}
    for year in YEARS:
        key = str(year)
        v17 = 0.95 * v11_prediction(oof[key]) + 0.05 * logistic[key]
        raw[year] = (w_v17 * v17 + w_form * form[key] + w_context * context[key]
                     + w_network * network[key] + w_catboost * catboost[key]
                     + w_extra * (extra[key] if extra is not None else 0.0))
    pieces = []
    for year in YEARS:
        if year == 2022:
...
def main():
    ...
    # LightGBM reads categoricals from pandas `category` dtype. The categories must be
    # identical across folds, so they are fixed once on the full column here.
    frames = {}
    for variant, columns in column_sets.items():
        work = features[columns].copy()
        for name in categorical:
            work[name] = work[name].astype(str).astype("category")
        ...
        for name, config in CONFIGS.items():
            ...
            for year in YEARS:
                ...
                model = lgb.LGBMClassifier(
                    **config, random_state=42, n_jobs=6, verbosity=-1)
    ...
    baseline = np.clip(
        blend(BASE + (0.0,), oof, logistic, form, context, network, catboost,
              None, raw_frame) + DRIFT_WEIGHT * term, 0, 1)
    ...
    for key in predictions:
        for w_extra in LIGHTGBM_WEIGHTS:
            # Take the new component's share from the remaining V17 layer, then Form.
            w_v17 = max(0.0, BASE[0] - w_extra)
            w_form = round(BASE[1] - max(0.0, w_extra - BASE[0]), 4)
            label = f"{key}_lgb{w_extra:.2f}"
            candidate = np.clip(
                blend((round(w_v17, 4), w_form) + BASE[2:] + (w_extra,),
                      oof, logistic, form, context, network, catboost,
                      predictions[key], raw_frame) + DRIFT_WEIGHT * term, 0, 1)
            ...
            metrics["season_bootstrap"] = {
                ...
                for year in YEARS}
            metrics["weights"] = {
                "v17": round(w_v17, 4), "form": w_form, "context": BASE[2],
                "network": BASE[3], "catboost": BASE[4], "lightgbm": w_extra}
    ...
    OUTPUT.write_text(json.dumps({
        "experiment": "V118_lightgbm_component",
        "baseline": f"V116 promoted ({CATBOOST_SOURCE} cb0.14) on top of V114",
        "baseline_weights": {"v17": BASE[0], "form": BASE[1], "context": BASE[2],
                             "network": BASE[3], "catboost": BASE[4]},
        "rationale": (
            "LightGBM and CatBoost are correlated tree learners over the same columns, "
            "so LightGBM is tested as an increment over the CatBoost blend rather than "
            "over plain V114, which would double-count shared signal."
        ),
        "configs": CONFIGS,
        "feature_variants": list(FEATURE_VARIANTS),
        "categorical_features": categorical,
        "lightgbm_weights": list(LIGHTGBM_WEIGHTS),
    ...
    print("\nranked by 2024 bootstrap lower bound (vs V116 blend):")
