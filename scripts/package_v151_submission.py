"""Package V151: a game_type segment added to the calibration.

Only the calibration artifact differs from V138. The inference script
iterates over `calibration["corrections"]`, so `v138_script.py` is
reused unmodified and every model file is byte-identical to V138.

This is the first submission that installs a package, so `requirements.txt` is no
longer only documentation. An installation failure does not count against the
daily quota, which is why trying is nearly free; a failure after `script.py`
starts does count, which is why the load gate below runs on the `cbm` too.
"""
...
def main():
    files = {
        Path("submission_src/v138_script.py"): "script.py",
        ...
        Path("artifacts/v151_calibration.joblib"): "model/v25_calibration.joblib",
        Path("artifacts/v114_network.joblib"): "model/v114_network.joblib",
        Path("artifacts/v138_factorization.joblib"): "model/v138_factorization.joblib",
        Path("artifacts/v117_catboost.cbm"): "model/v117_catboost.cbm",
        Path("artifacts/v117_catboost_meta.joblib"): "model/v117_catboost_meta.joblib",
    ...
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v151.zip")
