"""Package V193: average the network over nine draws and Form over six.

Context, CatBoost and the factorization network are unchanged, and the feature
frame is V175's 105 columns exactly -- V187 lost 2.59 with the extra encoding groups, so
they are not here. Three artifacts change: nine network bundles, six Form models beside
the untouched Context model, and the calibration, refitted because both averaged
components moved.

Timing is the check that matters most here. Inference now runs the network nine times and
Form six times on every row, so the full-frame run is re-measured against the 600-second
limit rather than inherited.
...
def main():
    files = {
        Path("submission_src/v193_script.py"): "script.py",
        ...
        Path("artifacts/v193_feature_models.joblib"): "model/v193_feature_models.joblib",
        Path("artifacts/v193_calibration.joblib"): "model/v25_calibration.joblib",
    ...
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v193.zip")
