...
OUTPUT = Path("artifacts/v140_high_latent_slot_metrics.json")
NAMES6 = ("v17", "form", "context", "network", "catboost", "factorization")
BASE = (0.00, 0.32, 0.21, 0.20, 0.27, 0.00)
CONTEXT_TOTAL = 0.21
HANDOVERS = (0.05, 0.07, 0.09, 0.12, 0.16, 0.21)
...
def main():
    ...
    factorization = {}
    for path in ("artifacts/v130c_interaction_network_predictions.joblib",
                 "artifacts/v130b_interaction_network_predictions.joblib"):
        for key, values in joblib.load(path)["predictions"].items():
            factorization.setdefault(key, values)
    ...
    results = {}
    print(f"{'candidate':>22} {'ctx':>5} {'fm':>5} {'min':>7} {'avg':>7} {'2022':>7} "
          f"{'2023':>8} {'2024':>7} {'blocks':>7} {'pass':>5}")
    for variant in ('latent8', 'latent16', 'latent24'):
        for handover in HANDOVERS:
            ...
            print(f"{label:>22} {context_weight:5.2f} {handover:5.2f} "
                  f"{min(t['season_points']):7.2f} {t['average_points']:7.2f} "
    ...
    promoted = max(eligible,
                   key=lambda l: min(results[l]["three_season"]["season_points"]),
                   default=None)
    ...
    OUTPUT.write_text(json.dumps({
        "experiment": "V140_high_latent_context_slot",
