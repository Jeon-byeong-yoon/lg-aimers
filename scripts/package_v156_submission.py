"""Package V156: V154 with finer experience bins in the calibration segment.

Only the calibration artifact differs. `v154_script.py` reads the bin edges
and labels out of the bundle, so it is reused unmodified and every model
file is byte-identical to V154.
...
def main():
    files = {
        Path("submission_src/v154_script.py"): "script.py",
        ...
        Path("artifacts/v156_calibration.joblib"): "model/v25_calibration.joblib",
    ...
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v156.zip")
