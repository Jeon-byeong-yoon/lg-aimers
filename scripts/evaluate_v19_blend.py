"""Test hierarchical ExtraTrees variants as additions to the V17 baseline."""
...
def v17_prediction(oof_item, logistic_prediction):
    return 0.95 * v11_prediction(oof_item) + 0.05 * logistic_prediction


def fold_score(oof, logistic, candidate, frame, history_years, valid_year, weight):
    train_y, train_base, train_new, indices = [], [], [], []
    for year in history_years:
        item = oof[str(year)]
        train_y.append(item["target"].astype(float))
        train_base.append(v17_prediction(item, logistic[str(year)]))
    ...
    valid_p = ((1 - weight) * v17_prediction(valid, logistic[str(valid_year)])
...
def main():
    ...
    candidates = joblib.load("artifacts/v19_hierarchical_extra_predictions.joblib")
    frame = pd.read_csv(
    ...
    Path("artifacts/v19_blend_metrics.json").write_text(
