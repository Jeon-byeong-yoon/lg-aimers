"""V201: is scale 225 a ridge or an isolated spike?

V200 rebuilt the instrument and it worked: the measured null -- two scale-300 draws against
each other, 36 pairs -- has sd 8.67 on the 2024 fold, so V160's and V162's readings of
plus or minus 1.1 were **0.13 standard deviations** and the whole reliability-scale decision
was noise. Pairing at the same seed cut that sd from 8.67 to 1.07, a factor of 8.1, because
the scale changes one column out of 105 and identical seeds start the two fits from
identical weights.

With the clean instrument one scale stands out:
...
2024 at 225 is +1.43 against a standard error of 0.55 -- 2.6 SE, the first significant
component-level effect this project has measured. But the 2024 column reads -0.16, +1.43,
0, +0.07, -0.29: **not monotone, with 225 alone sticking out.** Among four scales and three
seasons that is twelve readings, and one at 2.6 SE arises by chance about eleven percent of
the time.

So the same question V173 asked of the platoon term: ridge or spike. Scales 190 and 260 sit
either side of 225. If they gain too, there is a real optimum there and the incumbent 300 is
wrong. If they sit at zero, 225 was the eleven percent.

Identical design to V200 -- paired at the same seed, five seeds, Form untouched because it
is bit-identical across scales.
"""
...
OUTPUT = Path("artifacts/v201_reliability_ridge_metrics.json")
PREDICTIONS = Path("artifacts/v201_reliability_ridge_predictions.joblib")
...
def main():
    ...
    OUTPUT.write_text(json.dumps({
        "experiment": "V201_reliability_ridge",
        "baseline": "V199 (Public 1083.2461959655), feature reliability scale 300",
        "why": ("V200 found scale 225 gaining 2024 by +1.43 +- 0.55 while its neighbours "
                "150 and 300 sit at zero; twelve readings were taken, so an isolated "
                "2.6 SE spike arises by chance about eleven percent of the time"),
