"""Package V175: resolve the platoon segment by two-strike state.

Every model file is still V161's, byte for byte. Two files change: the
calibration bundle, whose fourth segment becomes `pitcher_id x batter_hand x
two_strike` at renormalised weights, and `script.py`, which must now derive the
`two_strike` column. That derivation is a per-row function of one official column and
happens after every feature is built, so no model sees it and no evaluation row touches
another.

Because `script.py` changed, the row-independence and full-frame timing checks are not
inherited from V169 and are re-run before submitting.
...
def main():
    files = {
        Path("submission_src/v175_script.py"): "script.py",
        ...
        Path("artifacts/v175_calibration.joblib"): "model/v25_calibration.joblib",
    ...
    output = Path("submissions/by_submitter/전병윤_RF_기준선/submit_v175.zip")
