...
제공된 안내에는 자동 생성 경로를 `data/`라고 설명하면서, 유의사항에는 `open/` 디렉토리에서 데이터를 읽으라고 표기되어 있어 서로 일치하지 않습니다. 현재 베이스라인 코드와 실제 제출 예시의 경로를 우선 확인하고, 불명확하면 `[DACON 답변 요청]`으로 운영진에 확인해야 합니다. 경로를 임의로 하나로 단정하지 않습니다.
